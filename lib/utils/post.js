...
export function getRecommendPost(post, allPosts, count = 6) {
  ...
  for (let i = 0; i < allPosts.length; i++) {
    const p = allPosts[i]
    if (p.id === post.id || !p?.type?.includes?.('Post')) {
      continue
    }
...
export async function processPostData(props, from) {
  ...
  if (allPosts && allPosts.length > 0) {
    let index = allPosts.indexOf(props.post)
    if (index < 0 && props.post?.id) {
      index = allPosts.findIndex(p => p?.id === props.post.id)
    }
    props.prev =
      index >= 0

...
function isNotionRateLimitError(e) {
  return (
    e?.status === 429 ||
...
/**
 * 调用接口，失败会重试；429 时使用指数退避，避免连续触发限流
 * @param {*} id
 * @param {*} retryAttempts
 */
export async function getPageWithRetry(id, from, retryAttempts = 5) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      const rateLimited = isNotionRateLimitError(e)
      // 429：5s 起指数退避；其它错误：1s
      const backoffMs = rateLimited
        ? Math.min(60_000, 5000 * Math.pow(2, 5 - retryAttempts))
        : 1000
      if (rateLimited) {
        console.warn(
          `[API] Notion 限流(429)，${backoffMs}ms 后重试，剩余 ${retryAttempts - 1} 次`
        )
      }
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  // 使用局部变量，避免对参数重新赋值（部分打包/严格模式边界下可能触发异常）
  let blockIds = Array.isArray(ids) ? ids : ids != null ? [ids] : []
  ...
  for (let i = 0; i < blockIds.length; i += batchSize) {
    ...
    console.log('[API-->>请求] Fetching missing blocks', batch.length)
    const start = new Date().getTime()
    const pageChunk = await notionAPI.getBlocks(batch)
    const end = new Date().getTime()
    console.log(
      `[API<<--响应] 耗时:${end - start}ms Fetching missing blocks count:${batch.length} `
    )
